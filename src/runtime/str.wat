...
(module
  ...
  ;; str_write_to_mem : (ref $Str) -> (i32 offset, i32 length)
  ;; Write string bytes to linear memory and return (offset, length).
  ;; For $StrDataImpl, returns the existing data-section pointer (zero copy).
  ;; For $StrBytesImpl, copies bytes to a scratch region at the end of
  ;; linear memory (growing if needed).
  ;; The scratch region is ephemeral — valid only until the next call.
  (func $str_write_to_mem (export "str_write_to_mem")
    (param $str (ref $Str))
    (result i32 i32)
    ...
    (block $not_data
      (block $is_data (result (ref $StrDataImpl))
        (br $not_data
          (br_on_cast $is_data (ref $Str) (ref $StrDataImpl)
            (local.get $str))))
      ;; Cast succeeded — return existing offset+length.
      (return
        (struct.get $StrDataImpl $offset)
        (struct.get $StrDataImpl $length
          (ref.cast (ref $StrDataImpl) (local.get $str)))))

    ;; Must be $StrBytesImpl — copy to linear memory.
    (local.set $bytes
      (struct.get $StrBytesImpl $bytes
        (ref.cast (ref $StrBytesImpl) (local.get $str))))
    (local.set $length (array.len (local.get $bytes)))

    ;; Use end of current memory as scratch area.
    (local.set $scratch_offset
      (i32.mul (memory.size) (i32.const 65536)))

    ;; Grow memory if needed.
    (local.set $pages_needed
      (i32.div_u
        (i32.add (local.get $length) (i32.const 65535))
        (i32.const 65536)))
    (if (i32.gt_u (local.get $pages_needed) (i32.const 0))
      (then
        (if (i32.eq (memory.grow (local.get $pages_needed)) (i32.const -1))
          (then (unreachable)))))
    ...
    (block $done
      (loop $copy
        (br_if $done
          (i32.ge_u (local.get $i) (local.get $length)))
        (i32.store8
          (i32.add (local.get $scratch_offset) (local.get $i))
          (array.get_u $ByteArray (local.get $bytes) (local.get $i)))
        (local.set $i
          (i32.add (local.get $i) (i32.const 1)))
        (br $copy)))

    (local.get $scratch_offset)
    (local.get $length)
  )
